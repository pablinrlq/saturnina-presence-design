...
export const Route = createFileRoute("/_authenticated/clientes")({
  head: () => ({
    meta: [
      { title: "Clientes — Saturnina Admin" },
      { name: "description", content: "Cadastro e histórico de clientes Saturnina." },
      { property: "og:title", content: "Clientes — Saturnina Admin" },
      { property: "og:description", content: "CRM de clientes Saturnina." },
  ...
  const create = useMutation({
    mutationFn: async (v: z.infer<typeof clientSchema>) => {
      const { data, error } = await supabase
        .from("clients")
        .insert({ ...v, birthday: v.birthday || null })
        .select()
        .single();
      if (error) throw error;
      return data;
    ...
    >
      <div className="grid gap-10 xl:grid-cols-[1fr_1.3fr]">
        <section>
          <label className="flex items-center gap-3 border-b border-primary-foreground/25 pb-2">
            ...
            <input
              ...
              className="w-full bg-transparent text-sm outline-none placeholder:text-primary-foreground/30"
            />
          </label>
          <ul className="mt-4">
            {isLoading && <li className="py-6 text-sm text-primary-foreground/50">Carregando…</li>}
            {!isLoading && filtered.length === 0 && (
              <li className="py-10 text-sm text-primary-foreground/50">
                Nenhuma cliente encontrada. Cadastre a primeira em “Nova cliente”.
              </li>
            )}
            {filtered.map((c) => (
              <li key={c.id}>
                <button
                  ...
                  className={`flex w-full items-baseline justify-between border-b border-primary-foreground/10 px-2 py-4 text-left transition-colors hover:bg-primary-foreground/5 ${selected === c.id ? "bg-primary" : ""}`}
                >
        ...
        <section>
          {current ? (
            ...
            <p className="border border-primary-foreground/10 p-10 text-sm text-primary-foreground/50">
              Selecione uma cliente para ver histórico, preferências e observações.
            </p>
          )}
        </section>
      </div>
      <Dialog open={creating} onOpenChange={setCreating}>
        <DialogContent className="border-primary-foreground/10 bg-brand-ink text-primary-foreground">
          <DialogHeader>
            <DialogTitle className="font-display text-3xl font-normal">Nova cliente</DialogTitle>
          </DialogHeader>
...
function ClientForm({
  ...
  submitting: boolean;
  onSubmit: (v: z.infer<typeof clientSchema>) => void;
}) {
  ...
  const field = (name: keyof Client, label: string, type = "text") => (
    <label className="block">
      ...
      />
      {errors[name] && <span className="text-xs text-brand-pink">{errors[name]}</span>}
    </label>
  );
  return (
    <form onSubmit={handle} className="grid gap-5 sm:grid-cols-2">
      ...
      <label className="sm:col-span-2">
        <span className={adminLabel}>Preferências</span>
        <textarea
          name="preferences"
          rows={2}
      ...
      <label className="sm:col-span-2">
        <span className={adminLabel}>Observações</span>
        <textarea
          name="notes"
          rows={2}
      ...
      <Button type="submit" variant="soft" disabled={submitting} className="sm:col-span-2">
...
function ClientDetail({ client }: { client: Client }) {
  ...
  const { data: history = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("appointments")
        .select("id, starts_at, status, notes, services(name), professionals(name)")
  ...
  const update = useMutation({
    mutationFn: async (v: z.infer<typeof clientSchema>) => {
      const { error } = await supabase
        .from("clients")
        .update({ ...v, birthday: v.birthday || null })
        .eq("id", client.id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const upcoming = history
    .filter((h) => new Date(h.starts_at) > new Date() && h.status !== "cancelled")
    .at(-1);
  const fmt = (d: string) =>
    new Date(d).toLocaleString("pt-BR", { dateStyle: "medium", timeStyle: "short" });
    ...
    <div>
          ...
          <dl className="mt-6 grid gap-5 text-sm sm:grid-cols-2">
            {[
              ...
              [
                "Aniversário",
                client.birthday
                  ? new Date(client.birthday + "T12:00").toLocaleDateString("pt-BR")
                  : "",
              ],
              ["Próximo atendimento", upcoming ? fmt(upcoming.starts_at) : ""],
              ["Serviços realizados", String(done.length)],
            ].map(([k, v]) => (
              <div key={k}>
      ...
      <div className="mt-4 border-t border-primary-foreground/20">
        ...
        {!isLoading && history.length === 0 && (
          <p className="py-6 text-sm text-primary-foreground/50">Sem agendamentos vinculados.</p>
        )}
        {history.map((h) => (
          <div
            key={h.id}
            className="grid grid-cols-[1fr_auto] gap-2 border-b border-primary-foreground/10 py-4 text-sm sm:grid-cols-[160px_1fr_1fr_100px]"
          >
            <span className="text-primary-foreground/60">{fmt(h.starts_at)}</span>
            <span>{h.services?.name ?? "Serviço"}</span>
            <span className="hidden text-primary-foreground/50 sm:block">
              {h.professionals?.name ?? "—"}
            </span>
